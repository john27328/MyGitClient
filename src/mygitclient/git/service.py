...
class GitService(QObject):
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        self._latest_stash_diff_request: dict[tuple[Path, str, str], int] = {}
    ...
    def request_commit_diff(
        ...
        *,
        ...
        self._runners.add(runner)
        request_id = next(self._request_ids)
        self._latest_commit_diff_request[(repository, commit_oid, path)] = request_id
    ...
    def request_ref_comparison_diff(
        ...
        *,
        ...
        self._runners.add(runner)
        request_id = next(self._request_ids)
        self._latest_comparison_diff_request[(repository, base_ref, compare_ref, path)] = request_id
    ...
    def request_stash_diff(self, repository: Path, stash: StashInfo, path: str) -> GitRunner:
        ...
        self._runners.add(runner)
        request_id = next(self._request_ids)
        self._latest_stash_diff_request[(repository, stash.oid, path)] = request_id
    ...
    def _handle_ref_comparison_diff(self, result: object) -> None:
        ...
        key = (repository, base_ref, compare_ref, path)
    ...
    def _handle_commit_diff(self, result: object) -> None:
        ...
        if request is None or not isinstance(result, GitResult):
            ...
            return
        repository, commit_oid, path, request_id = request
        key = (repository, commit_oid, path)
